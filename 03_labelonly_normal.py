...
from models import *
from mia_labelonly_hopskipjump import AdversaryTwo_HopSkipJump
from utils import dataset_split

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data-dir', default='../data', type=str)
    parser.add_argument('--fname', default='03_labelonly_normal', type=str)
    parser.add_argument('--seed', default=0, type=int)
    return parser.parse_args()
args = get_args()

def AdversaryTwo(data_loader, fw):
    torch.cuda.empty_cache()
    
    targetmodel = torch.load(f"materials/01_train_target_shadow/target.pth", weights_only=False).cuda().eval()
    AdversaryTwo_HopSkipJump(args, targetmodel, data_loader, fw)
